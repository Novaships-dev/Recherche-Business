...
> ⚠️ **ÉTAT AU 30/07/2026 : le fichier `PROMPT_RECHERCHE_SECTEUR.md` est ANNONCÉ
> MAIS ABSENT.** Vérifié à la racine, sur tout le disque, et l'arbre git est
> propre : il n'a jamais été ajouté. Tant qu'il n'est pas présent, les sections
> marquées `À COMPLÉTER` ci-dessous ne peuvent pas être renseignées sans
> invention. Ne pas les combler de mémoire ni par déduction.
...
2. `À COMPLÉTER` — défini dans `PROMPT_RECHERCHE_SECTEUR.md`, fichier absent
   du repo au 30/07/2026. **Ne pas inventer.**
3. `À COMPLÉTER` — idem.

## 3. Hiérarchie des sources d'avis

`À COMPLÉTER` — définie dans `PROMPT_RECHERCHE_SECTEUR.md`, fichier absent du
repo au 30/07/2026. Aucun élément du repo actuel ne permet de la reconstituer.
**Ne pas inventer un ordre de priorité.**
...
- **Réutilisation du code de l'app BTP** (champ du barème de score) :
  `INCONNU`. Aucune information n'a jamais été fournie sur cette application.
  Le prompt annoncé sur ce point n'est pas arrivé.
